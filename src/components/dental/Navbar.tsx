...
export function Navbar() {
  const { cartCount, wishlistIds, isAuthenticated } = useStore();
          ...
          <div className="flex items-center gap-3">
            ...
            {isAuthenticated ? (
              <Button
                ...
                variant="tertiary"
                size="icon"
                className={`hidden ${compactActionClassName} lg:inline-flex`}
              >
                <Link href="/account/dashboard" data-testid="link-account" aria-label={t("nav.account")}>
        ...
        >
          <nav className="mx-auto flex max-w-[1344px] flex-col gap-1">
            ...
            >
              <span className="inline-flex items-center gap-3">
                {isAuthenticated ? (
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--xd-gold)] text-[11px] font-bold text-[#050505]">
                    AH
                  </span>
                ) : (
